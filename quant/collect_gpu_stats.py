...
def main():
    ...
    def collect_gpu_stats(model, data_loader, num_batch=200):
        model.eval()
        with torch.no_grad():
    ...
    torch.cuda.memory._record_memory_history(
       True, trace_alloc_max_entries=MAX_NUM_OF_MEM_EVENTS_PER_SNAPSHOT, trace_alloc_record_context=True
    )

    collect_gpu_stats(model, test_loader, 200)

    try:
        snapshot = torch.cuda.memory._snapshot()
        with open('./file.pickle', 'wb') as f:
            dump(snapshot, f)
            logger.info("Pickle file successfully generated!")
